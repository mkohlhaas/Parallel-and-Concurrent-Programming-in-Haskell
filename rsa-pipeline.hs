...
import Control.Monad.Par.Scheds.Trace (Par, runPar)
import Data.ByteString.Lazy.Char8 (ByteString)
import qualified Data.ByteString.Lazy.Char8 as B
import Data.List ()
import Stream (Stream, streamFold, streamFromList, streamMap)
...
encrypt n e = streamMap (B.pack . show . power e n . code)

decrypt ∷ Integer → Integer → Stream ByteString → Par (Stream ByteString)
decrypt n d = streamMap (B.pack . decode . power d n . integer)

integer ∷ ByteString → Integer
integer b | Just (i, _) ← B.readInteger b = i
integer _ = error "Should not happen!"
