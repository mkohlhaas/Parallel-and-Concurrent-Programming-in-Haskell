...
import Text.XML.Light (CData (CData, cdData), Content (Elem, Text), Element (Element, elContent, elName), QName (QName, qName), parseXML)
...
translateText text fromLang toLang = do
  ...
  pure $ concatMap getString (parseXML (UTF8.toString r))
...
getLangs [Elem Element {elName = QName {qName = "ArrayOfstring"}, elContent = strs}] = map getString strs
getLangs _ = error "Wrong input format!"

getString ∷ Content → String
getString (Elem Element {elName = QName {qName = "string"}, elContent = [Text CData {cdData = str}]}) = str
getString _ = error "Wrong input format!"
