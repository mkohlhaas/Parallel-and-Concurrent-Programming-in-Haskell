...
-- A document index and search program.
-- Use it like this:
...
mkIndex i s =
  Map.fromListWith Set.union [(B.concat (L.toChunks w), Set.singleton i) | w ← ws]
  where
    ws = L.splitWith (not . isAlphaNum) s

search ∷ DocIndex → [Word] → DocSet
search index words = foldr1 Set.intersection (map lookup words)
  where
    lookup w = Map.findWithDefault Set.empty w index
...
main = do
  ...
  let -- indices is a separate index for each (numbered) document
      indices ∷ [DocIndex]
