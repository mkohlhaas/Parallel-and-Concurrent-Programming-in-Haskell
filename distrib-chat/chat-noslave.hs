...
import Control.Distributed.Process hiding (Message, finally, handleMessage, mask, proxy)
  ...
  (ProcessMonitorNotification _ pid _) = do
    ...
    liftIO $
      atomically $ do
        ...
        clientmap ← readTVar clients
        let now_disconnected (ClientRemote RemoteClient {..}) = clientHome == pid
