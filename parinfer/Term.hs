...
import Shows (Shows, showsChar, showsParenIf, showsString)
...
showsTerm d (Abs x v) = showsParenIf (d > 0) (showsString "\\" . showsString x . showsAbs v)
showsTerm d (App t u) = showsParenIf (d > 1) (showsTerm 1 t . showsChar ' ' . showsTerm 2 u)
showsTerm d (Let x u v) =
  showsParenIf
    (d > 0)
